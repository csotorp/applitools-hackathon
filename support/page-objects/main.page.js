...
export const main = {
  ...
  shouldShowOptions(options) {
    options.sort 
      ? els.sortSel().should('be.visible').find(':selected').should('contain.text', 'Sort by popularity')
      : els.sortSel().should('not.be.visible');
    options.filters ? els.filtersIco().should('be.visible') : els.filtersIco().should('not.be.visible');
    options.filters && options.filtersLbl
      ? els.filtersLbl().should('be.visible').and('have.text', 'Filters') 
